...
5. Отчёт оформлять по структуре владельца (образец:
   `docs/trends-report-2026-08-01.md`): постановка задачи → источники
   (со статусом каждого: TikTok/IG/YT Shorts/Reddit/Meta Ad Library) →
   методика → таблица финалистов (просмотры/лайки/комменты/дата/почему) →
   почему победитель → итог с паттернами. Фильтр свежести ≤12 мес:
   старше — флагировать ⚠ и подтверждать паттерн свежим примером.
   Цифры ТОЛЬКО из реальной выгрузки — никаких оценок по памяти.
6. Конечная выдача сценария владельцу — «Сюжет по кадрам» (образец:
   `docs/storyboard-vtech-pro-honest-timeline.md`): заголовок-хук →
   «Источник идеи (не самопридумано)» с метриками → хронометраж →
   по сценам: название-функция (ХУК-БОЛЬ/ПЕРЕХВАТ/...), таймкоды,
   🎬 съёмка / 🤖 AI-пара, START-кадр, END-кадр, Титр, Голос,
   «Из победителя: …» → кэпшн с механикой → «Честно о рисках» →
   статус и следующий гейт. Раскадровку дублировать в
   `docs/storyboard-<scriptId>.md`, данные — в `ugc-scripts.json`.
7. К каждому финалисту — психологический разбор по таймкодам (образец:
   `docs/reel-breakdowns-2026-08-01.md`, формат владельца 01.08): блоки
   Hook 0–2s / 2–5s / 5–10s / финал — что происходит (описание, не
   цитата) → «почему сработало» (механика внимания: незавершённость,
   социальная идентификация, когнитивная нагрузка, обрыв на пике) →
   список факторов «почему набрал». Это ДОПОЛНЕНИЕ к провенанс-отчёту,
   не замена.
